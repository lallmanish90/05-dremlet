...
def main():
    ...
    with col1:
        if st.button("🔍 Run Verification", type="primary"):
            try:
                ...
                # Validate results before storing (handle namespace issues)
                if not (isinstance(results, VerificationResults) or 
                        (hasattr(results, '__class__') and results.__class__.__name__ == 'VerificationResults')):
                    st.error(f"Internal error: Expected VerificationResults, got {type(results)}")
                    st.error(f"Results content: {results}")
                    results = VerificationResults()
                    results.errors.append(f"Verification returned unexpected type: {type(results)}")
                ...
                st.error(f"Verification failed with error: {str(e)}")
                # Create error results
                error_results = VerificationResults()
                error_results.errors.append(f"Verification failed: {str(e)}")
                st.session_state.verification_results = error_results
    ...
    if st.session_state.verification_results:
        ...
        # Debug: Check the type of results
        try:
            # Check if it's a VerificationResults object (handle namespace issues)
            if not (isinstance(results, VerificationResults) or 
                    (hasattr(results, '__class__') and results.__class__.__name__ == 'VerificationResults')):
                st.error(f"Error: Expected VerificationResults object, got {type(results)}")
                st.error(f"Results content: {results}")
                st.session_state.verification_results = None
                return
            
            # Check for errors - with additional safety
            if hasattr(results, 'errors') and results.errors:
                st.error("Verification encountered errors:")
                for error in results.errors:
                    st.error(error)
                return
            elif not hasattr(results, 'errors'):
                st.error(f"Results object missing 'errors' attribute. Type: {type(results)}")
                st.error(f"Available attributes: {dir(results)}")
                st.session_state.verification_results = None
                return
        except Exception as e:
            st.error(f"Error while checking results: {str(e)}")
            st.error(f"Results type: {type(results)}")
            st.session_state.verification_results = None
            return
