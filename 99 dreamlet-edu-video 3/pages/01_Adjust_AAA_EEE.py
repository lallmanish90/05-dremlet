...
def get_processed_directory(course_path: str = None) -> str:
    ...
    Otherwise, returns the top-level processed_aaa_eee (for finding existing files).
    """
    if course_path:
        return os.path.join(course_path, "processed_aaa_eee")
    return os.path.join(get_input_directory(), "processed_aaa_eee")
...
def move_file_to_processed(file_path: str) -> bool:
    ...
    E.g., input/CourseName/Lecture01/01-AAA.md -> input/CourseName/Lecture01/processed_aaa_eee/01-AAA.md
    """
    try:
        ...
        # Ensure the new processed directory exists
...
def main():
    ...
    # Add checkbox for Artifact-C creation
    create_artifact_c = st.checkbox("✅ Create 'artifact-c' folder with 'XX-c' files?", value=True, help="If checked, a folder named 'artifact-c' will be created at the project root, containing copies of all 'XX-c' (Slide Content) files, organized by course.")
    ...
    with st.expander("ℹ️ What does this tool do? (Click to expand)"):
        ...
        **Key Features:**
        - **File Movement**: Original AAA/EEE files are moved to `processed_aaa_eee/` folder after splitting to prevent re-processing
